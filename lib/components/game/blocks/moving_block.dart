...
class MovingBlock extends CollisionBlock with HasGameReference<PixelAdventure> {
  ...
  final double _terminalVelocity = 50;
  ...
  Future<void> onLoad() async {
    ...
    ///TODO hacecr bien la hitbox de la caja
    add(
      RectangleHitbox(
  ...
  void _applyGravity(double dt) {
    velocity.y += _gravity;
    velocity.y = velocity.y.clamp(0, _terminalVelocity);
    position.y += velocity.y * dt;
  }

  bool isBelow(PositionComponent other) {
    return other.position.y > position.y + size.y - 1;
  }
  //
